<script setup lang="ts">
    ...
    type PresetType = 'Aura' | 'Lara' | 'Nora';

    const { layoutConfig, isDarkTheme } = useLayout();

    // --- Datos y Opciones ---
    const presets: Record<PresetType, any> = { Aura, Lara, Nora };
    const preset = ref<PresetType>(layoutConfig.preset as PresetType);
    const presetOptions = ref<PresetType[]>(Object.keys(presets) as PresetType[]);
    ...
    const saveConfig = () => {
        try {
            ...
            console.error('Error saving theme configuration to localStorage:', e);
        }
    };
    ...
    const loadConfig = () => {
        try {
            ...
            // Aplicar valores a layoutConfig (Estos son reactivos)
            layoutConfig.primary = config.primary;
            layoutConfig.surface = config.surface;
            layoutConfig.preset = config.preset;
            preset.value = config.preset as PresetType;

            // Aplicar Dark Mode
            // Actualizar el estado reactivo
            layoutConfig.darkTheme = config.darkTheme;

            // Sincronizar la clase 'app-dark' inmediatamente en el DOM
            if (config.darkTheme) {
    ...
    /**
     * Aplica el tema completo: preset + colores primarios + superficie
     */
    ...
    // Watcher para guardar cuando cambie isDarkTheme desde el botón del topbar
    watch(isDarkTheme, () => {
        saveConfig();
    });
