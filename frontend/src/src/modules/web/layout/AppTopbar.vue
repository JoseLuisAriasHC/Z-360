...
<template>
    <div :class="{ 'modal-backdrop': isMenuOpen }" @mouseleave="handleMouseLeave">
        <MegaMenu
            :model="items"
            class="bg-surface-0 top-0 left-0 z-50 shadow-md"
            :pt="{
                start: { class: 'mr-auto' },
                overlay: { class: 'lg:mt-4' },
                rootList: {
            ...
            <template #start>
                <router-link :to="{ name: 'home' }" class="layout-topbar-logo">
                    <svg class="w-20 h-16" viewBox="0 0 54 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
...
<style scoped>
    ...
    /* ➡️ Estilos para el efecto de fondo oscuro/modal */
    .modal-backdrop::after {
        ...
        /* Desenfoque (opcional, simula profundidad) */
        backdrop-filter: blur(2px);
        z-index: 40; /* Debe estar por debajo del MegaMenu (z-50) pero por encima del resto del contenido */
        pointer-events: none; /* Permite hacer clic en elementos que estén por debajo si es necesario, aunque el MegaMenu debería estar manejando la interacción */
        transition:
            background-color 0.3s ease,
            backdrop-filter 0.3s ease;
    }

    /* Asegúrate de que el MegaMenu esté por encima del backdrop */
    .p-megamenu {
